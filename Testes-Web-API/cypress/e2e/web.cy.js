...
describe('Seatecnologia - Cadastro', () => {
  ...
  it('CT-001 - Efetuar cadastro de funcionário com dados válidos', { tags: ['@smoke', '@regression'] }, () => { // Cadastro de funcionário via Web
  ...
  it('CT-002 - Efetuar cadastro de funcionário ativo com dados válidos', { tags: '@regression' }, () => { // Cadastro de funcionário ativo via Web
    cy.get('.c-kUQtTK').click()
  ...
  it('CT-003 - Efetuar cadastro de funcionário com idade menor de 18 anos', { tags: '@known-issue' }, () => { // Cadastro de funcionário com idade menor de 18 anos via Web
    cy.get('.c-kUQtTK').click()
    cy.get('.ant-switch').click()
    cy.get('[name="name"]').type(employeeData.nome)
    cy.get('[name="cpf"]').type(employeeData.cpf)
  ...
  it('CT-004 - Efetuar cadastro de funcionário com data de nascimento futura', { tags: '@known-issue' }, () => {  // Cadastro de funcionário com data de nascimento futura via Web
    cy.get('.c-kUQtTK').click()
    cy.get('.ant-switch').click()
    cy.get('[name="name"]').type(employeeData.nome)
    cy.get('[name="cpf"]').type(employeeData.cpf)
  ...
  it('CT-005 - Não permitir cadastro de funcionário com CPF duplicado', { tags: '@known-issue' }, () => {  // Cadastro de funcionário com CPF duplicado via Web
...
describe('Seatecnologia - Validação', () => {
  ...
  it('CT-006 - Validar preenchimento obrigatório do campo Nome', { tags: '@regression' }, () => { // Validação do preenchimento obrigatório do campo Nome via Web
  cy.get('.c-kUQtTK').click()  
  cy.get('[name="cpf"]').type(employeeData.cpf)
  cy.get('[name="rg"]').type(employeeData.rg)
  cy.get('input[name="birthDay"]').type(employeeData.dataNasc)
  ...
  it('CT-007 - Validar preenchimento obrigatório do campo CPF', { tags: '@regression' }, () => { // Validação do preenchimento obrigatório do campo CPF via Web
  cy.get('.c-kUQtTK').click()  
  cy.get('[name="name"]').type(employeeData.nome)
  cy.get('[name="rg"]').type(employeeData.rg)
  cy.get('input[name="birthDay"]').type(employeeData.dataNasc)
  ...
  it('CT-008 - Validar preenchimento obrigatório do campo RG', { tags: '@regression' }, () => { // Validação do preenchimento obrigatório do campo RG via Web
  cy.get('.c-kUQtTK').click()  
  cy.get('[name="name"]').type(employeeData.nome)
  cy.get('[name="cpf"]').type(employeeData.cpf)
  cy.get('input[name="birthDay"]').type(employeeData.dataNasc)
  ...
  it('CT-009 - Validar preenchimento obrigatório do campo Data de nascimento', { tags: '@regression' } ,() => { // Validação do preenchimento obrigatório do campo Data de nascimento via Web
  cy.get('.c-kUQtTK').click()  
  cy.get('[name="name"]').type(employeeData.nome)
  cy.get('[name="cpf"]').type(employeeData.cpf)
  cy.get('[name="rg"]').type(employeeData.rg)
  ...
  it('CT-010 - Validar preenchimento obrigatório do campo Número do CA', { tags: '@regression' }, () => { // Validação do preenchimento obrigatório do campo Número do CA via Web
  cy.get('.c-kUQtTK').click()  
  cy.get('[name="name"]').type(employeeData.nome)
  cy.get('[name="cpf"]').type(employeeData.cpf)
  cy.get('[name="rg"]').type(employeeData.rg)
  ...
  it('CT-011 - Validar quantidade mínima de caracteres no campo CPF', { tags: '@regression' }, () => { // Validação da quantidade mínima de caracteres no campo CPF via Web
  cy.get('.c-kUQtTK').click()  
  cy.get('[name="cpf"]').should('have.attr', 'minlength', '11')

  })

  it('CT-012 - Validar inserção de emojis nos campos de cadastro', { tags: '@known-issue' }, () => { // Validação da inserção de emojis nos campos de cadastro via Web
    cy.get('.c-kUQtTK').click()
    cy.get('.ant-switch').click()
